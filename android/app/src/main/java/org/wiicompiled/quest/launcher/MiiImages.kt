...
/**
 * Mii pictures for My Miis and its editor, drawn by [MiiRenderer] off the main thread and kept
 * in a memory cache (MiiImagesSingletonService on the PC). Nothing is drawn until the player has
 * downloaded the Mii parts ([MiiRenderResource]); callers then show their placeholder.
 */
object MiiImages {
    ...
    /** Shows [mii]'s head in [view], [size] pixels square; a recycled view only takes the picture it asked for last. */
    fun head(context: Context, view: ImageView, mii: Mii, size: Int) {
        val copy = mii.copy()
        show(context, view, "head:$size:${copy.lookKey()}") { resource -> MiiRenderer.render(resource, copy, size) }
    }
    ...
    fun preview(context: Context, mii: Mii, size: Int, done: (Bitmap?) -> Unit) {
        ...
        previewer.execute {
            ...
            val bitmap = draw(app, key) { resource -> MiiRenderer.render(resource, copy, size) }
    ...
    private fun show(context: Context, view: ImageView, key: String, render: (FflResource) -> IntArray?) {
        ...
        renderers.execute {
            ...
            val bitmap = draw(app, key, render) ?: return@execute
            main.post { if (view.getTag(R.id.mii_image_key) == key) view.setImageBitmap(bitmap) }
        }
    }

    private fun draw(context: Context, key: String, render: (FflResource) -> IntArray?): Bitmap? {
        ...
        return try {
            ...
            Bitmap.createBitmap(pixels, size, size, Bitmap.Config.ARGB_8888).also { cache.put(key, it) }
        } catch (e: Exception) {
